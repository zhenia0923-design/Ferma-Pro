-- FERMA PRO: повна автоматична синхронізація складу, витрат і доходів
-- Запустити ПІСЛЯ supabase_migration.sql. Цей файл замінює попередній supabase_integration.sql.
...
create or replace function public.fp_sync_warehouse_use_finance() returns trigger language plpgsql security definer set search_path=public as $$
declare v_price numeric:=0; v_amount numeric:=0; v_name text; v_type text;
begin
if tg_op in ('UPDATE','DELETE') then delete from public.finance where user_id=old.user_id and description like '%[AUTO_WH_USE:'||old.id||']%'; end if;
if tg_op in ('INSERT','UPDATE') and new.movement_type='use' then
 select name,item_type into v_name,v_type from public.warehouse_items where id=new.item_id;
 select coalesce(sum(total_amount)/nullif(sum(quantity),0),0) into v_price from public.warehouse_movements where user_id=new.user_id and item_id=new.item_id and movement_type='in' and movement_date<=new.movement_date;
 v_price:=case when coalesce(new.unit_price,0)>0 then new.unit_price else v_price end;
 v_amount:=case when coalesce(new.total_amount,0)>0 then new.total_amount else new.quantity*v_price end;
 if v_amount>0 then insert into public.finance(user_id,operation_type,operation_date,category,amount,description) values(new.user_id,'expense',new.movement_date,case when v_type='medicine' then 'Ліки' else 'Корм/склад' end,v_amount,coalesce(new.notes,'Використання складу')||' [AUTO_WH_USE:'||new.id||']'); end if;
 return new; end if; return old; end $$;
drop trigger if exists trg_fp_warehouse_use_finance on public.warehouse_movements;
create trigger trg_fp_warehouse_use_finance after insert or update or delete on public.warehouse_movements for each row execute function public.fp_sync_warehouse_use_finance();
